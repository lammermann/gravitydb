...
if z ~= 0 then
  error("failed to generate lua bindings", 0)
end
...
local modules = table.join(".", os.matchdirs("dependencies/*"))
for _, module in ipairs(modules) do
  for _, name in ipairs { ".git" } do
    os.rmdir(path.join(module, name))
  end
  for _, name in ipairs { ".git", ".gitignore", ".gitmodules", ".travis.yml" } do
...
if kind == "binary" then
  ...
  if os.is("windows") then
    execQuiet("zip -9 -r %s release", name)
  else
    execQuiet("tar czvf %s release", name)
  end
